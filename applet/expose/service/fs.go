...
func (fs *FS) Create(ctx context.Context, dir string, mh *multipart.FileHeader) (*model.FS, error) {
	file, err := mh.Open()
	if err != nil {
		return nil, err
	}
...
func (fs *FS) Update(ctx context.Context, dir string, mh *multipart.FileHeader) (*model.FS, error) {
	file, err := mh.Open()
	if err != nil {
		return nil, err
	}
	...
	return repo.Upload(ctx, fp, file)
}
...
func (fs *FS) List(ctx context.Context, dir string) (model.FSs, error) {
	repo := fs.repo.FS()
	return repo.List(ctx, dir)
}
